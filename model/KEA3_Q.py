...
def sim_a_gtype(gtype_dict, idx=0, light = 100,P700_red_values=None):  
    ...
    if P700_red_values is not None:
        initial_sim_state_list[0]=P700_red_values[idx]
        print("count +1")
...
def do_stuff(LIGHT):
    ...
    for idx in range(10000):  # 循环从索引 0 到 999
        gtype_dict = {}
        gtype_df = sim_a_gtype(gtype_dict, idx=idx, light=LIGHT, P700_red_values=None)  # 获取单个仿真结果的 gtype_df
    ...
    combined_df.to_csv(f'./logs/combined_{LIGHT}_simulated.csv', index=False)
    print(f"All results for LIGHT {LIGHT} saved to combined_{LIGHT}_simulated.csv")
